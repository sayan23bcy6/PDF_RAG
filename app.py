import streamlit as st
import os
import sys
from pathlib import Path
from dotenv import load_dotenv
from session_manager import SessionManager
from pdf_processor import PDFProcessor
from rag_chain import RAGChain
from utils import setup_logger

load_dotenv()

def ensure_directory(dir_path: str) -> bool:
    """
    Robustly create directory with comprehensive error handling for Windows.
    Returns True if successful, False otherwise.
    """
    try:
        abs_path = os.path.abspath(dir_path)
        
        if not os.path.exists(abs_path):
            os.makedirs(abs_path, mode=0o777, exist_ok=True)
        
        if not os.path.isdir(abs_path):
            print(f"Error: {abs_path} exists but is not a directory")
            return False
        
        return True
    except PermissionError:
        print(f"Permission denied creating {dir_path}")
        return False
    except Exception as e:
        print(f"Error creating {dir_path}: {e}")
        return False

# Create necessary directories
print("Initializing application directories...")
required_dirs = ["logs", "data", "data/uploads", "data/chroma_db"]
for dir_path in required_dirs:
    success = ensure_directory(dir_path)
    if success:
        print(f"✓ Directory ready: {dir_path}")
    else:
        print(f"✗ Failed to create: {dir_path}")

# Setup logger
try:
    logger = setup_logger(__name__)
except Exception as e:
    print(f"Warning: Could not setup logger: {e}")
    logger = None

def initialize_app():
    """Initialize the Streamlit app state"""
    if 'session_manager' not in st.session_state:
        try:
            st.session_state.session_manager = SessionManager()
        except Exception as e:
            st.error(f"Failed to initialize SessionManager: {e}")
            st.stop()
    
    if 'current_session_id' not in st.session_state:
        try:
            st.session_state.current_session_id = st.session_state.session_manager.create_session()
        except Exception as e:
            st.error(f"Failed to create session: {e}")
            st.stop()
    
    if 'rag_chain' not in st.session_state:
        st.session_state.rag_chain = None
    
    if 'pdf_processor' not in st.session_state:
        st.session_state.pdf_processor = PDFProcessor()

def main():
    st.set_page_config(
        page_title="PDF RAG Chat Application",
        page_icon="📄",
        layout="wide",
        initial_sidebar_state="expanded"
    )
    
    st.title("PDF RAG Chat Application")
    st.write("Upload and chat with multiple PDF documents using AI-powered retrieval augmented generation.")
    
    initialize_app()
    
    session_manager = st.session_state.session_manager
    current_session_id = st.session_state.current_session_id
    
    with st.sidebar:
        st.header("Session Management")
        
        col1, col2 = st.columns(2)
        with col1:
            if st.button("New Session", use_container_width=True):
                st.session_state.current_session_id = session_manager.create_session()
                st.session_state.rag_chain = None
                st.rerun()
        
        with col2:
            if st.button("Delete Session", use_container_width=True):
                session_manager.delete_session(current_session_id)
                st.session_state.current_session_id = session_manager.create_session()
                st.session_state.rag_chain = None
                st.rerun()
        
        st.divider()
        
        st.subheader(f"Current Session ID")
        st.code(current_session_id, language="text")
        
        st.subheader("Session History")
        all_sessions = session_manager.list_sessions()
        
        if all_sessions:
            for session_id in all_sessions:
                session_data = session_manager.get_session(session_id)
                col1, col2 = st.columns([3, 1])
                
                with col1:
                    if st.button(
                        f"{session_id[:8]}... ({len(session_data.get('uploaded_files', []))} PDFs)",
                        use_container_width=True,
                        key=f"load_{session_id}"
                    ):
                        st.session_state.current_session_id = session_id
                        st.session_state.rag_chain = None
                        st.rerun()
                
                with col2:
                    if st.button("X", key=f"delete_{session_id}"):
                        session_manager.delete_session(session_id)
                        if session_id == current_session_id:
                            st.session_state.current_session_id = session_manager.create_session()
                            st.session_state.rag_chain = None
                        st.rerun()
        else:
            st.info("No previous sessions found.")
        
        st.divider()
        st.subheader("Application Settings")
        st.info(f"Max PDFs per session: {os.getenv('MAX_PDFS_PER_SESSION', 5)}")
    
    session_data = session_manager.get_session(current_session_id)
    
    col1, col2 = st.columns([2, 1])
    
    with col1:
        st.header("PDF Upload")
        uploaded_files = st.file_uploader(
            "Upload PDF files",
            type="pdf",
            accept_multiple_files=True,
            key=f"file_uploader_{current_session_id}"
        )
        
        max_pdfs = int(os.getenv('MAX_PDFS_PER_SESSION', 5))
        current_pdfs = len(session_data.get('uploaded_files', []))
        
        if uploaded_files:
            st.write(f"Processing {len(uploaded_files)} file(s)...")
            
            if current_pdfs + len(uploaded_files) > max_pdfs:
                st.error(f"Cannot upload more than {max_pdfs} PDFs per session. Current: {current_pdfs}")
            else:
                progress_bar = st.progress(0)
                status_text = st.empty()
                
                pdf_processor = st.session_state.pdf_processor
                all_documents = []
                
                for idx, uploaded_file in enumerate(uploaded_files):
                    try:
                        status_text.text(f"Processing file {idx + 1}/{len(uploaded_files)}: {uploaded_file.name}")
                        
                        documents = pdf_processor.process_pdf(
                            uploaded_file,
                            current_session_id
                        )
                        all_documents.extend(documents)
                        
                        progress_bar.progress((idx + 1) / len(uploaded_files))
                        
                    except Exception as e:
                        st.error(f"Error processing {uploaded_file.name}: {str(e)}")
                        if logger:
                            logger.error(f"Error processing PDF: {str(e)}")
                
                if all_documents:
                    session_manager.update_session(
                        current_session_id,
                        uploaded_files=[f.name for f in uploaded_files]
                    )
                    
                    try:
                        status_text.text("Building vector store and RAG chain...")
                        rag_chain = RAGChain()
                        rag_chain.build(all_documents, current_session_id)
                        st.session_state.rag_chain = rag_chain
                        
                        st.success(f"Successfully processed {len(uploaded_files)} PDF(s)")
                        progress_bar.empty()
                        status_text.empty()
                        
                    except Exception as e:
                        st.error(f"Error building RAG chain: {str(e)}")
                        if logger:
                            logger.error(f"Error building RAG chain: {str(e)}")
    
    with col2:
        st.header("Session Info")
        session_info = session_manager.get_session(current_session_id)
        
        st.metric("Uploaded PDFs", len(session_info.get('uploaded_files', [])))
        st.metric("Chat Messages", len(session_info.get('messages', [])))
        
        if session_info.get('uploaded_files'):
            with st.expander("View Uploaded Files"):
                for file in session_info.get('uploaded_files', []):
                    st.write(f"- {file}")
    
    st.divider()
    
    if st.session_state.rag_chain is None:
        if len(session_data.get('uploaded_files', [])) > 0:
            st.info("Session has PDFs. Rebuilding RAG chain...")
            try:
                rag_chain = RAGChain()
                rag_chain.load(current_session_id)
                st.session_state.rag_chain = rag_chain
            except Exception as e:
                st.warning(f"Could not rebuild RAG chain: {str(e)}")
    
    st.header("Chat with PDFs")
    
    if st.session_state.rag_chain:
        chat_container = st.container()
        
        with chat_container:
            session_messages = session_data.get('messages', [])
            
            for message in session_messages:
                with st.chat_message(message['role']):
                    st.write(message['content'])
        
        user_input = st.chat_input("Ask a question about your PDFs...")
        
        if user_input:
            with st.chat_message("user"):
                st.write(user_input)
            
            try:
                with st.spinner("Generating response..."):
                    rag_chain = st.session_state.rag_chain
                    
                    response = rag_chain.invoke(
                        user_input,
                        current_session_id
                    )
                    
                    with st.chat_message("assistant"):
                        st.write(response)
                    
                    session_manager.add_message(current_session_id, "user", user_input)
                    session_manager.add_message(current_session_id, "assistant", response)
                    
                    st.rerun()
                    
            except Exception as e:
                st.error(f"Error generating response: {str(e)}")
                if logger:
                    logger.error(f"Error generating response: {str(e)}")
    else:
        st.info("Please upload PDF files to start chatting.")

if __name__ == "__main__":
    main()